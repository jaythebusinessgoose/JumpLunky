...
--------------------------------------
---- DISABLE JOURNAL // TODO(jaythebusinessgoose): Remove this chunk when recap screen is fixed when journal disabled.
--------------------------------------

local level_started = false
set_callback(function()
	if not level_started then return end
	-- Enable the journal if there is no player or the player is dead. This is due to the recap being considered a journal and
	-- not showing when the journal is disabled.
	if not players[1] or players[1].health <= 0 then
